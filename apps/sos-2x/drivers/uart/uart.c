...
void uart_init(void) {
	...
	uart_state = (uart_state_t*)sys_malloc(sizeof(uart_state_t)*2);
	sys_set_state_pointer((void*)(&uart_state[0]), (void**)(&state[0]));
	sys_set_state_pointer((void*)(&uart_state[1]), (void**)(&state[1]));
...
int8_t uart_startTransceiverRx( uint8_t rx_len, uint8_t flags) {
	...
	if (flags & UART_SOS_MSG_FLAG) {
		...
		state[RX]->buff = sys_malloc(rx_len);
	...
	switch (state[RX]->state) {
		...
		case UART_PROTOCOL:
			...
			switch (byte_in) {
				...
				case HDLC_RAW:
					if ((state[RX]->buff = sys_malloc(UART_MAX_MSG_LEN)) != NULL) {
						...
						if (state[RX]->idx == SOS_MSG_HEADER_SIZE) {
							...
							if (state[RX]->msgLen < UART_MAX_MSG_LEN) {
								if (state[RX]->msgLen != 0) {
									state[RX]->buff = (uint8_t*)sys_malloc(state[RX]->msgLen);
									if (state[RX]->buff != NULL) {
